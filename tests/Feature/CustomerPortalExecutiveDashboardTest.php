...
class CustomerPortalExecutiveDashboardTest extends TestCase
{
    public function test_customer_portal_dashboard_contains_approved_executive_sections_and_drilldowns(): void
    {
        ...
        foreach (['Service Request Status','Work Orders Status','Asset Health','Monthly Activity','Contracts & SLA','Financial Summary','Request Trend','Needs Attention','Quick Actions','New Service Request','Emergency Maintenance','Request Quotation','Request Spare Parts','Technical Consultation','Messages & Support','Get in touch with our support team.'] as $section) {
            $this->assertStringContainsString($section, $view);
        }

        foreach (["route('customer.portal')","route('customer.section','requests')","route('customer.section','work-orders')","route('customer.section','assets')","route('customer.section','contracts')","route('customer.section','visits')","route('customer.section','invoices')","route('customer.actions')","route('customer.inbox')","route('public.request-service'"] as $target) {
