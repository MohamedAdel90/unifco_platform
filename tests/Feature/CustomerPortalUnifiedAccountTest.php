...
class CustomerPortalUnifiedAccountTest extends TestCase
{
    ...
    public function test_customer_pages_share_the_unified_portal_shell(): void
    {
        ...
        $this->actingAs($user)->get('/customer/profile?lang=en')->assertOk()->assertSee('UNIFIED CUSTOMER ACCOUNT')->assertSee('Login &amp; Security',false)->assertSee('Language &amp; Preferences',false);
        ...
        if($request){$this->actingAs($user)->get(route('customer.service-requests.show',$request))->assertOk()->assertSee('UNIFIED CUSTOMER ACCOUNT')->assertSee('Related Records');}
    }

    public function test_global_search_finds_only_records_from_the_authenticated_customer(): void
    {
        $this->seed(WorkflowTestUsersSeeder::class);$user=User::where('email','workflow.customer@unifco.local')->firstOrFail();$customer=Customer::findOrFail($user->customer_id);
        ...
        $this->actingAs($user)->get('/customer/search?q=Search+Pump')->assertOk()->assertSee('Customer Search Pump')->assertDontSee('Foreign Search Pump');
    }

    public function test_unified_request_form_prefills_the_authenticated_customer_and_supports_presets(): void
    {
        $this->seed(WorkflowTestUsersSeeder::class);$user=User::where('email','workflow.customer@unifco.local')->firstOrFail();$customer=Customer::findOrFail($user->customer_id);
        $this->actingAs($user)->get('/request-service?emergency=1')->assertOk()->assertSee('value="'.$customer->customer_code.'"',false)->assertSee('const preset={emergency:true',false);
        $this->actingAs($user)->get('/request-service?quotation=1&subtype=parts')->assertOk()->assertSee('SPARE_PARTS_QUOTE',false)->assertSee('quotation:true',false);
        $this->actingAs($user)->get('/request-service?consultation=1')->assertOk()->assertSee('consultation:true',false);
    }

    public function test_customer_dashboard_presents_a_decision_ready_operating_summary(): void
    {
        $this->seed(WorkflowTestUsersSeeder::class);$user=User::where('email','workflow.customer@unifco.local')->firstOrFail();
        $this->actingAs($user)->get('/customer?days=30')->assertOk()->assertSee('Open Requests')->assertSee('Work Orders Status')->assertSee('Asset Health')->assertSee('Contracts & SLA',false)->assertSee('Financial Summary')->assertSee('Request Trend')->assertSee('Technical Consultation');
    }
}
