...
return new class extends Migration {
    public function up(): void
    {
        ...
        Schema::create('work_order_assignments', function (Blueprint $t) {
            ...
            $t->string('dispatch_status',30)->default('SCHEDULED');
            $t->timestamp('dispatched_at')->nullable();
            $t->timestamp('accepted_at')->nullable();
            $t->timestamp('arrived_at')->nullable();
            $t->text('dispatcher_notes')->nullable();
            $t->timestamps();
            $t->unique(['work_order_id','employee_id']);
            $t->index(['employee_id','scheduled_start']);
        });
        ...
        Schema::create('inspection_templates', function (Blueprint $t) {
            ...
            $t->foreignId('tenant_id')->constrained()->cascadeOnDelete();
            $t->foreignId('organization_id')->nullable()->constrained()->nullOnDelete();
            $t->string('template_no',60);
            $t->string('name');
            $t->json('checklist');
            $t->string('status',20)->default('ACTIVE');
            $t->timestamps();
            $t->unique(['tenant_id','template_no']);
        });
        ...
        Schema::create('inspections', function (Blueprint $t) {
            ...
            $t->foreignId('inspection_template_id')->nullable()->constrained('inspection_templates')->nullOnDelete();
            $t->foreignId('employee_id')->nullable()->constrained('employees')->nullOnDelete();
            $t->string('inspection_no',60);
            ...
            $t->timestamps();
            $t->unique(['tenant_id','inspection_no']);
        });
        ...
        Schema::create('ai_interactions', function (Blueprint $t) {
            ...
            $t->json('recommended_actions')->nullable();
            $t->string('result',20)->default('ANSWERED');
            $t->timestamps();
        });
    }
