...
<div class="ops-shell">
 ...
 <section class="card table-wrap">
   <h3>Technician Utilization Snapshot</h3><div class="sub">Current active field assignments inside the manager's Work Order scope.</div>
   <table class="table"><thead><tr><th>Technician</th><th>Active Assignments</th><th>In Progress</th><th>Arrived</th></tr></thead><tbody>
   @forelse($utilization as $row)<tr><td><strong>{{ $row['employee'] }}</strong></td><td>{{ $row['active_assignments'] }}</td><td>{{ $row['in_progress'] }}</td><td>{{ $row['arrived'] }}</td></tr>@empty<tr><td colspan="4" class="muted">No active field assignments in the current scope.</td></tr>@endforelse
   </tbody></table>
 </section>
</div>
@endsection
