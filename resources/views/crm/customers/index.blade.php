...
@foreach($customers as $c)<tr><td>{{ $c->customer_code }}</td><td>{{ $c->name }}</td><td>{{ $c->email }}</td><td>{{ $c->status }}</td><td><a href="{{ route('crm.customers.edit',$c) }}">Edit</a> · <a href="{{ route('crm.customers.portal',$c) }}">Portal & Contract 360</a> @if($c->status==='ACTIVE')<form style="display:inline" method="POST" action="{{ route('crm.customers.block',$c) }}">@csrf<button>Block</button></form>@endif</td></tr>@endforeach
