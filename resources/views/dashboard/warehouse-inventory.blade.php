...
<style>
.wh-summary{margin-top:16px;background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:15px}.wh-summary-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:10px}.wh-summary-head h3{margin:0;color:#071f4d;font-size:13px}.wh-summary-head small{font-size:8px;color:#758399}.wh-summary-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(115px,1fr));gap:8px}.wh-summary-card{display:block;background:#f7f9fc;border-radius:9px;padding:9px;color:#071f4d}.wh-summary-card span{display:block;font-size:7px;color:#758399}.wh-summary-card b{font-size:16px}.wh-summary-card.alert b{color:#c51b34}.wh-summary-card.ready b{color:#176a43}@media(max-width:600px){.wh-summary-grid{grid-template-columns:1fr 1fr}}
</style>
<section class="wh-summary"><div class="wh-summary-head"><span><h3>Warehouse & Field Inventory · المستودع وقطع الغيار</h3><small>Central warehouse + technician vehicles + site stock + work order part requests.</small></span><a class="btn secondary" href="{{ route('inventory.warehouse.index') }}">Open Spare Parts Control Center</a></div><div class="wh-summary-grid">
...
<a class="wh-summary-card {{ $warehouseSummary['part_requests_pending']?'alert':'' }}" href="{{ route('inventory.warehouse.index') }}"><span>Technician Requests</span><b>{{ $warehouseSummary['part_requests_pending'] }}</b></a>
<a class="wh-summary-card {{ $warehouseSummary['part_requests_reserved']?'ready':'' }}" href="{{ route('inventory.warehouse.index') }}"><span>Reserved / Picking</span><b>{{ $warehouseSummary['part_requests_reserved'] }}</b></a>
<a class="wh-summary-card {{ $warehouseSummary['part_requests_awaiting_receipt']?'alert':'' }}" href="{{ route('inventory.warehouse.index') }}"><span>Awaiting Technician Receipt</span><b>{{ $warehouseSummary['part_requests_awaiting_receipt'] }}</b></a>
