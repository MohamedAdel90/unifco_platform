...
<p>Use this space for design screenshots and PDFs you want to share temporarily. After upload, copy the public link and paste it into this conversation.</p>
<form class="upload-zone" method="POST" action="{{ route('admin.temporary-files.store') }}" enctype="multipart/form-data">@csrf
<label><strong>Choose file</strong><input type="file" name="file" accept="image/jpeg,image/png,image/webp,image/gif,application/pdf" required></label>
<button class="btn" type="submit">Upload Temporary File</button>
</form>
<p>Accepted: JPG, PNG, WEBP, GIF, PDF · Maximum size: 20 MB.</p>
...
@if($file['is_image'])<img src="{{ $file['public_url'] }}" alt="{{ $file['original_name'] }}">@else<span class="pdf-preview">PDF</span>@endif
</a>
<div class="file-info">
