...
@endif
            ...
            @if($section === 'quotations')
                ...
                <div class="page-head quote-page-head"><div><div class="eyebrow">Commercial & Contracts</div><h2>Quotations</h2><p>Commercial proposals for your customer account. Review quotations, download documents and take action.</p></div><a class="btn quote-request" href="{{ route('public.request-service',['customer'=>$customer->customer_code,'quotation'=>1]) }}">+ &nbsp; Request Quotation</a></div>
                ...
                <div class="card quote-tools">
                    <label class="quote-search"><input id="quote-search" type="search" placeholder="Search quotations by number, type or related request..."></label>
                    <label class="filter"><select id="quote-status"><option value="">All Statuses</option><option>DRAFT</option><option>SENT</option><option>UNDER_REVIEW</option><option>REVISION_REQUESTED</option><option>APPROVED</option><option>REJECTED</option></select></label>
                    <label class="filter"><select id="quote-type"><option value="">All Types</option>@foreach($quotationTypes->keys()->sort() as $type)<option value="{{ strtolower($type) }}">{{ $type }}</option>@endforeach</select></label>
                    <button type="button" class="filter-button" id="quote-reset">Reset</button>
                </div>
                ...
                @else
                    <div class="card table-card quote-desktop"><div class="table-wrap"><table class="table quote-table"><thead><tr><th>#</th><th>Quotation</th><th>Type / Related Request</th><th>Revision</th><th>Value</th><th>Updated</th><th>Status</th><th>Customer Action</th></tr></thead><tbody>@foreach($quotations as $quotation)<tr data-quote-row data-search="{{ strtolower($quotation->quotation_no.' '.($quotation->quotation_type ?? $quotation->type ?? 'general quotation').' '.$quotation->status) }}" data-status="{{ strtoupper((string)$quotation->status) }}" data-type="{{ strtolower(trim((string)($quotation->quotation_type ?? $quotation->type ?? 'General Quotation'))) }}"><td>{{ $loop->iteration }}</td><td><strong>{{ $quotation->quotation_no }}</strong><small>Created {{ $quotation->created_at?->format('d M Y') }}</small></td><td><b>{{ $quotation->quotation_type ?? $quotation->type ?? 'General Quotation' }}</b></td><td>R{{ $quotation->revision_no }}</td><td>{{ number_format((float)$quotation->amount,2) }} {{ $quotation->currency }}</td><td>{{ $quotation->updated_at?->format('d M Y') }}<small>{{ $quotation->updated_at?->format('h:i A') }}</small></td><td><span class="pill">{{ $quotation->status }}</span></td><td>@if($canDecideQuotation && in_array($quotation->status,['SENT','UNDER_REVIEW','REVISION_REQUESTED']))<form class="quote-decision" method="POST" action="{{ route('customer.quotations.decision',$quotation) }}">@csrf<select name="decision"><option value="APPROVE">Approve</option><option value="REVISION">Revision</option><option value="REJECT">Reject</option></select><input name="notes" placeholder="Notes"><button class="btn">Submit</button></form>@else<span class="pill">{{ $readOnly ? 'Read only' : 'No action required' }}</span>@endif</td></tr>@endforeach</tbody></table></div><div class="quote-footer">Showing 1 to {{ $quotationCount }} of {{ $quotationCount }} quotations <span class="pill">1</span></div></div>
                    <div class="quote-mobile">@foreach($quotations as $quotation)<article class="card quote-card" data-quote-row data-search="{{ strtolower($quotation->quotation_no.' '.($quotation->quotation_type ?? $quotation->type ?? 'general quotation').' '.$quotation->status) }}" data-status="{{ strtoupper((string)$quotation->status) }}" data-type="{{ strtolower(trim((string)($quotation->quotation_type ?? $quotation->type ?? 'General Quotation'))) }}"><div class="quote-card-top"><span class="quote-kpi-icon">@include('customer.partials.icon',['name'=>'quotations'])</span><div><strong>{{ $quotation->quotation_no }}</strong><small>{{ $quotation->quotation_type ?? $quotation->type ?? 'General Quotation' }}</small></div><span class="pill">{{ $quotation->status }}</span></div><div class="quote-card-meta"><span>R{{ $quotation->revision_no }}</span><span>{{ number_format((float)$quotation->amount,2) }} {{ $quotation->currency }}</span><span>{{ $quotation->updated_at?->format('d M Y') }}</span></div>@if($canDecideQuotation && in_array($quotation->status,['SENT','UNDER_REVIEW','REVISION_REQUESTED']))<form class="quote-decision" method="POST" action="{{ route('customer.quotations.decision',$quotation) }}">@csrf<select name="decision"><option value="APPROVE">Approve</option><option value="REVISION">Revision</option><option value="REJECT">Reject</option></select><input name="notes" placeholder="Notes"><button class="btn">Submit</button></form>@endif</article>@endforeach</div>
                    <section class="quote-insights"><div class="card panel"><div class="panel-head"><h3>Quotations by Type</h3><span class="pill">{{ $quotationCount }} total</span></div><div class="quote-type-list">@foreach($quotationTypes as $type=>$items)<div><span>{{ $type }}</span><div><i style="width:{{ $quotationCount?round(($items->count()/$quotationCount)*100):0 }}%"></i></div><b>{{ $items->count() }}</b></div>@endforeach</div></div><div class="card panel"><div class="panel-head"><h3>Recent Quotations</h3></div>@foreach($quotations->sortByDesc('updated_at')->take(5) as $quotation)<div class="list-row"><i class="indicator"></i><div><div class="row-title">{{ $quotation->quotation_no }}</div><div class="row-sub">{{ $quotation->updated_at?->format('d M Y') }} · {{ $quotation->status }}</div></div></div>@endforeach</div><div class="card panel"><div class="panel-head"><h3>Quick Actions</h3></div><a class="list-row" href="{{ route('public.request-service',['customer'=>$customer->customer_code,'quotation'=>1]) }}"><i class="indicator"></i><div><div class="row-title">Request a New Quotation</div><div class="row-sub">Submit a quotation request</div></div></a><a class="list-row" href="{{ route('customer.inbox') }}"><i class="indicator"></i><div><div class="row-title">Need Help?</div><div class="row-sub">Contact UNIFCO support</div></div></a></div></section>
                @endif
                <script>document.addEventListener('DOMContentLoaded',()=>{const q=document.getElementById('quote-search'),s=document.getElementById('quote-status'),t=document.getElementById('quote-type'),r=document.getElementById('quote-reset'),rows=[...document.querySelectorAll('[data-quote-row]')];const apply=()=>{const text=(q?.value||'').toLowerCase(),status=(s?.value||'').toUpperCase(),type=(t?.value||'').toLowerCase();rows.forEach(row=>row.style.display=(!text||row.dataset.search.includes(text))&&(!status||row.dataset.status===status)&&(!type||row.dataset.type===type)?'':'none')};q?.addEventListener('input',apply);s?.addEventListener('change',apply);t?.addEventListener('change',apply);r?.addEventListener('click',()=>{q.value='';s.value='';t.value='';apply()})});</script>
                <style>
                @media(min-width:781px){body:has(.quote-page-head) .content{max-width:1080px}.quote-page-head{margin-bottom:10px}.quote-page-head h2{font-size:21px;margin-bottom:3px}.quote-page-head p{font-size:8px}.quote-page-head .eyebrow{font-size:7px;margin-bottom:5px}.quote-request{min-width:128px;padding:7px 10px;font-size:8px}.quote-kpis{gap:8px;margin-bottom:9px}.quote-kpi{padding:10px 11px;min-height:72px;gap:8px}.quote-kpi-icon{width:31px;height:31px;border-radius:9px;font-size:18px}.quote-kpi-icon .ui-icon{width:17px;height:17px}.quote-kpi b{font-size:17px}.quote-kpi strong{font-size:8px;margin:4px 0 2px}.quote-kpi small{font-size:6.5px}.quote-tools{padding:7px;gap:7px;margin-bottom:9px}.quote-search input,.quote-tools .filter select,.quote-tools .filter-button{height:30px;font-size:8px}.quote-desktop{padding:10px}.quote-table{font-size:8px}.quote-table th{font-size:7px}.quote-table th,.quote-table td{padding:7px 6px}.quote-table td strong{font-size:8px}.quote-table td small{font-size:6.5px}.quote-footer{padding:8px 10px;font-size:7px}.quote-insights{gap:9px;margin-top:9px}.quote-insights .panel{padding:11px}.quote-insights .panel-head{margin-bottom:9px}.quote-insights .panel-head h3{font-size:10px}.quote-insights .list-row{padding:6px 0}.quote-type-list{gap:7px}.quote-type-list>div{grid-template-columns:90px 1fr 18px;font-size:7px}}
                .quote-page-head h2{display:flex;align-items:center;gap:8px}.quote-request{min-width:150px;text-align:center}.quote-kpis{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin-bottom:12px}.quote-kpi{padding:14px;min-height:100px;display:flex;gap:11px;align-items:flex-start}.quote-kpi-icon{width:38px;height:38px;border-radius:12px;background:#eaf3ff;color:var(--blue);display:grid;place-items:center;flex:0 0 auto;font-size:22px;font-weight:800}.quote-kpi-icon .ui-icon{width:20px;height:20px}.quote-kpi b{font-size:20px;line-height:1;display:block}.quote-kpi strong{display:block;font-size:9px;margin:6px 0 4px}.quote-kpi small{display:block;font-size:7.5px;color:var(--muted);line-height:1.35}.quote-kpi.draft .quote-kpi-icon{background:#fff4df;color:#d88a00}.quote-kpi.waiting .quote-kpi-icon,.quote-kpi.rejected .quote-kpi-icon{background:#fdebed;color:var(--red)}.quote-kpi.approved .quote-kpi-icon{background:#e8f8ee;color:#10a75a}.quote-tools{padding:10px;display:grid;grid-template-columns:minmax(250px,1.7fr) minmax(120px,.55fr) minmax(120px,.55fr) auto;gap:9px;margin-bottom:12px}.quote-search input{width:100%;height:34px;border:1px solid var(--line);border-radius:8px;padding:0 11px;font-size:9px}.quote-tools .filter select{width:100%}.quote-table td strong{display:block;color:var(--blue);font-size:9px}.quote-table td small{display:block;color:var(--muted);font-size:7px;margin-top:3px}.quote-decision{display:flex;gap:4px;align-items:center}.quote-decision select,.quote-decision input{height:29px;border:1px solid var(--line);border-radius:7px;font-size:8px;max-width:105px}.quote-decision .btn{padding:7px 8px;font-size:8px}.quote-footer{padding:11px 14px;display:flex;justify-content:space-between;align-items:center;color:var(--muted);font-size:8px;border-top:1px solid var(--line)}.quote-mobile{display:none}.quote-insights{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-top:12px}.quote-type-list{display:grid;gap:9px}.quote-type-list>div{display:grid;grid-template-columns:110px 1fr 22px;gap:8px;align-items:center;font-size:8px}.quote-type-list>div>div{height:6px;background:#edf1f5;border-radius:999px;overflow:hidden}.quote-type-list i{height:100%;display:block;background:var(--blue);border-radius:inherit}.quote-empty{min-height:260px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:25px}.quote-empty strong{font-size:14px;margin:12px 0 5px}.quote-empty p{font-size:9px;color:var(--muted);margin:0 0 14px}.quote-card{padding:12px}.quote-card-top{display:grid;grid-template-columns:auto 1fr auto;gap:9px;align-items:center}.quote-card-top strong{display:block;color:var(--blue);font-size:10px}.quote-card-top small{display:block;color:var(--muted);font-size:8px;margin-top:3px}.quote-card-meta{display:flex;justify-content:space-between;gap:6px;margin-top:11px;padding-top:10px;border-top:1px solid var(--line);font-size:8px;color:var(--muted)}
                ...
                @media(max-width:780px){.quote-page-head{flex-direction:column;align-items:stretch}.quote-request{width:100%}.quote-kpis{grid-template-columns:1fr 1fr}.quote-tools{grid-template-columns:1fr 1fr}.quote-search{grid-column:1/-1}.quote-desktop{display:none}.quote-mobile{display:grid;gap:8px}.quote-insights{grid-template-columns:1fr}.quote-insights>div:last-child{grid-column:auto}}
                @media(max-width:520px){
                .quote-page-head{gap:10px;margin-bottom:12px}.quote-page-head h2{font-size:27px;margin-bottom:4px}.quote-page-head p{font-size:8px;line-height:1.45}.quote-request{height:36px;padding:9px 12px}
                .quote-kpis{grid-template-columns:repeat(4,minmax(0,1fr));gap:6px;margin-bottom:9px}.quote-kpi{min-height:72px;padding:8px 6px;gap:5px;align-items:center;justify-content:center;text-align:center;flex-direction:column}.quote-kpi:first-child{grid-column:1/-1;min-height:64px;flex-direction:row;text-align:left;justify-content:flex-start;padding:9px 12px}.quote-kpi-icon{width:30px;height:30px;border-radius:9px;font-size:17px}.quote-kpi-icon .ui-icon{width:16px;height:16px}.quote-kpi b{font-size:18px;margin:0}.quote-kpi strong{font-size:7.5px;margin:3px 0 0;line-height:1.15}.quote-kpi small{display:none}
                .quote-tools{padding:8px;gap:6px;margin-bottom:9px;grid-template-columns:1fr 1fr auto}.quote-search{grid-column:1/-1}.quote-search input{height:32px;font-size:8px}.quote-tools .filter select{height:32px;font-size:8px}.quote-tools .filter-button{width:auto;min-width:58px;height:32px;padding:0 9px;font-size:8px}
                .quote-mobile{gap:6px}.quote-card{padding:9px 10px;border-radius:12px}.quote-card-top{gap:7px}.quote-card-top .quote-kpi-icon{width:31px;height:31px}.quote-card-top strong{font-size:9px}.quote-card-top small{font-size:7px;margin-top:2px}.quote-card-top .pill{font-size:7px;padding:5px 7px}.quote-card-meta{margin-top:7px;padding-top:7px;font-size:7.5px}.quote-card .quote-decision{margin-top:8px;display:grid;grid-template-columns:1fr 1fr}.quote-card .quote-decision input{max-width:none}.quote-card .quote-decision .btn{grid-column:1/-1}
                .quote-insights{gap:7px;margin-top:8px}.quote-insights>div:nth-child(2),.quote-insights>div:nth-child(3){display:none}.quote-insights .panel{padding:10px}.quote-insights .panel-head{margin-bottom:7px}.quote-insights .panel-head h3{font-size:10px}.quote-type-list{gap:6px}.quote-type-list>div{grid-template-columns:82px 1fr 18px;font-size:7px}
                }                </style>
...
@endif
            ...
            @if($section === 'reports')
                ...
                <div class="page-head reports-page-head">
                    <div><div class="eyebrow">Customer Records</div><h2>Reports</h2><p>Technical reports across all customer sites and assets.</p></div>
                    <div class="reports-head-actions"><button class="btn reports-date" type="button">01 Jan 2026 – {{ now()->format('d M Y') }}</button><a class="btn reports-export" href="#reports-table">⇩ Export Report</a><a class="btn reports-generate" href="{{ route('public.request-service',['customer'=>$customer->customer_code]) }}">＋ Generate Report</a></div>
                </div>
                <section class="reports-kpis">
                    <div class="card reports-kpi"><span class="reports-kpi-icon">@include('customer.partials.icon',['name'=>'reports'])</span><div><small>Total Reports</small><b>{{ $reportCount }}</b><em>All technical reports</em></div></div>
                    <div class="card reports-kpi"><span class="reports-kpi-icon">@include('customer.partials.icon',['name'=>'visits'])</span><div><small>This Month</small><b>{{ $reportThisMonth }}</b><em>Generated this month</em></div></div>
                    <div class="card reports-kpi"><span class="reports-kpi-icon">@include('customer.partials.icon',['name'=>'sites'])</span><div><small>Report Types</small><b>{{ $reportTypes->count() }}</b><em>Across visible records</em></div></div>
                    <div class="card reports-kpi"><span class="reports-kpi-icon">@include('customer.partials.icon',['name'=>'assets'])</span><div><small>Customer Sites</small><b>{{ $sites->count() }}</b><em>Authorized sites</em></div></div>
                </section>
                <div class="card panel reports-tools">
                    <label class="reports-search"><span>Search reports</span><input id="reports-search" type="search" placeholder="Search reports, request number, site, asset or technician..."></label>
                    <label class="filter"><span>Report Type</span><select id="reports-type"><option value="">All Types</option>@foreach($reportTypes->keys()->sort() as $type)<option value="{{ strtolower($type) }}">{{ $type }}</option>@endforeach</select></label>
                    <label class="filter"><span>Site</span><select><option>All Sites</option>@foreach($sites as $site)<option>{{ $site->name }}</option>@endforeach</select></label>
                    <label class="filter"><span>Status</span><select><option>All Statuses</option><option>Completed</option><option>Under Review</option></select></label>
                    <button type="button" class="btn soft reports-reset" id="reports-reset">↻ Reset</button>
                    <button type="button" class="filter-button">⌁ Apply Filters</button>
                </div>
                @if($visitReports->isEmpty())
                    <div class="card reports-empty"><span class="reports-empty-icon">@include('customer.partials.icon',['name'=>'reports'])</span><strong>No reports available yet</strong><p>Technical reports will appear here automatically after completed service visits and maintenance activity.</p></div>
                @else
                    <div class="card table-card reports-desktop" id="reports-table"><div class="table-wrap"><table class="table reports-table"><thead><tr><th>#</th><th>Report No.</th><th>Report Type</th><th>Technician</th><th>Date</th><th>Status</th><th>Actions</th></tr></thead><tbody>@foreach($visitReports as $report)<tr data-report-row data-search="{{ strtolower($report->report_no.' '.$report->visit_type.' '.$report->technician_name) }}" data-type="{{ strtolower(trim((string)($report->visit_type ?: 'Other'))) }}"><td>{{ $loop->iteration }}</td><td><strong>{{ $report->report_no }}</strong></td><td><span class="pill">{{ $report->visit_type ?: 'Technical Report' }}</span></td><td>{{ $report->technician_name ?: '—' }}</td><td>{{ $report->visit_date?->format('d M Y') ?: '—' }}</td><td><span class="pill green">Completed</span></td><td><a class="pill" href="{{ route('customer.visits.pdf',$report) }}">Download PDF</a></td></tr>@endforeach</tbody></table></div></div>
                    <div class="reports-mobile">@foreach($visitReports as $report)<article class="card report-card" data-report-row data-search="{{ strtolower($report->report_no.' '.$report->visit_type.' '.$report->technician_name) }}" data-type="{{ strtolower(trim((string)($report->visit_type ?: 'Other'))) }}"><div class="report-card-head"><span class="reports-kpi-icon">@include('customer.partials.icon',['name'=>'reports'])</span><div><strong>{{ $report->report_no }}</strong><small>{{ $report->visit_type ?: 'Technical Report' }}</small></div><span class="pill green">Completed</span></div><div class="report-card-meta"><span>{{ $report->technician_name ?: 'Technician not assigned' }}</span><span>{{ $report->visit_date?->format('d M Y') ?: 'Date unavailable' }}</span></div><a class="btn" href="{{ route('customer.visits.pdf',$report) }}">Download PDF</a></article>@endforeach</div>
                    <section class="reports-insights">
                        <div class="card panel"><div class="panel-head"><h3>Reports by Type</h3><span class="pill">{{ $reportCount }} total</span></div><div class="reports-type-list">@foreach($reportTypes as $type => $items)<div><span>{{ $type }}</span><div class="reports-bar"><i style="width:{{ $reportCount ? round(($items->count()/$reportCount)*100) : 0 }}%"></i></div><b>{{ $items->count() }}</b></div>@endforeach</div></div>
                        <div class="card panel"><div class="panel-head"><h3>Recent Reports</h3></div><div class="activity-list">@foreach($visitReports->sortByDesc('visit_date')->take(5) as $report)<div class="list-row"><i class="indicator"></i><div><div class="row-title">{{ $report->report_no }}</div><div class="row-sub">{{ $report->visit_type ?: 'Technical Report' }} · {{ $report->visit_date?->format('d M Y') }}</div></div><a class="pill" href="{{ route('customer.visits.pdf',$report) }}">PDF</a></div>@endforeach</div></div>
                    </section>
                @endif
                <script>document.addEventListener('DOMContentLoaded',()=>{const q=document.getElementById('reports-search'),t=document.getElementById('reports-type'),r=document.getElementById('reports-reset'),rows=[...document.querySelectorAll('[data-report-row]')];const apply=()=>{const s=(q?.value||'').toLowerCase(),type=(t?.value||'').toLowerCase();rows.forEach(row=>row.style.display=(!s||row.dataset.search.includes(s))&&(!type||row.dataset.type===type)?'':'none')};q?.addEventListener('input',apply);t?.addEventListener('change',apply);r?.addEventListener('click',()=>{q.value='';t.value='';apply()})});</script>
                <style>
                    .reports-page-head{margin-bottom:18px}.reports-page-head h2{font-size:32px}.reports-page-head p{font-size:12px}.reports-page-head .eyebrow{font-size:9px}.reports-head-actions{display:flex;gap:10px;flex-wrap:wrap}.reports-head-actions .btn{min-height:40px;padding:10px 15px;font-size:10px}.reports-date{background:#fff;color:var(--ink);border:1px solid var(--line)}.reports-generate{background:var(--blue)}.reports-kpis{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-bottom:16px}.reports-kpi{padding:20px;display:flex;align-items:center;gap:16px;min-height:128px}.reports-kpi-icon,.reports-empty-icon{width:50px;height:50px;border-radius:14px;background:#eaf3ff;color:var(--blue);display:grid;place-items:center;flex:0 0 auto}.reports-kpi-icon .ui-icon,.reports-empty-icon .ui-icon{width:24px;height:24px}.reports-kpi small{display:block;font-size:11px;font-weight:800;color:#53647a}.reports-kpi b{display:block;font-size:32px;line-height:1;margin:8px 0}.reports-kpi em{display:block;font-style:normal;font-size:10px;color:var(--muted)}.reports-tools{display:grid;grid-template-columns:minmax(360px,2.1fr) repeat(3,minmax(160px,.75fr)) auto auto;align-items:end;gap:12px;margin-bottom:16px;padding:18px}.reports-search{flex:1}.reports-search span,.reports-tools .filter span{display:block;font-size:9px;color:var(--muted);font-weight:800;margin-bottom:6px}.reports-search input,.reports-tools .filter select{width:100%;height:40px;border:1px solid var(--line);border-radius:8px;padding:0 12px;font-size:10px}.reports-tools .btn,.reports-tools .filter-button{height:40px;padding:0 14px;font-size:10px}.reports-table{font-size:11px}.reports-table th{font-size:9px}.reports-table td{padding:13px 10px}.reports-table td strong{color:var(--blue)}.reports-mobile{display:none}.reports-empty{min-height:460px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:36px}.reports-empty-icon{width:66px;height:66px;border-radius:18px}.reports-empty-icon .ui-icon{width:32px;height:32px}.reports-empty strong{font-size:17px;margin:16px 0 6px}.reports-empty p{max-width:560px;color:var(--muted);font-size:11px;line-height:1.7;margin:0}.reports-insights{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-top:12px}.reports-type-list{display:grid;gap:10px}.reports-type-list>div{display:grid;grid-template-columns:130px 1fr 28px;align-items:center;gap:10px;font-size:9px}.reports-type-list b{text-align:right}.reports-bar{height:7px;background:#edf1f5;border-radius:999px;overflow:hidden}.reports-bar i{display:block;height:100%;background:var(--blue);border-radius:inherit}.report-card{padding:13px}.report-card-head{display:grid;grid-template-columns:auto 1fr auto;gap:10px;align-items:center}.report-card-head strong{display:block;font-size:11px;color:var(--blue)}.report-card-head small{display:block;font-size:8px;color:var(--muted);margin-top:3px}.report-card-meta{display:flex;justify-content:space-between;gap:10px;margin:11px 0;color:var(--muted);font-size:8px}.report-card .btn{display:block;text-align:center}
                    @media(max-width:1300px){.reports-tools{grid-template-columns:minmax(260px,1.5fr) repeat(2,minmax(140px,.7fr)) auto auto}.reports-tools .filter:nth-of-type(4){display:none}}@media(max-width:1000px){.reports-kpis{grid-template-columns:1fr 1fr}.reports-tools{display:flex;flex-wrap:wrap}.reports-search{flex-basis:100%}}
                    @media(max-width:780px){.reports-page-head{flex-direction:column;align-items:stretch}.reports-head-actions .btn{width:100%;text-align:center}.reports-tools{align-items:stretch;flex-wrap:wrap}.reports-search{flex-basis:100%}.reports-tools .filter{flex:1}.reports-desktop{display:none}.reports-mobile{display:grid;gap:8px}.reports-insights{grid-template-columns:1fr}.reports-empty{min-height:230px}}
                    @media(max-width:520px){.reports-page-head h2{font-size:27px}.reports-kpis{grid-template-columns:1fr 1fr;gap:8px}.reports-kpi{min-height:105px;padding:11px;gap:8px}.reports-kpi-icon{width:35px;height:35px}.reports-kpi b{font-size:22px}.reports-kpi em{font-size:7px}.reports-tools{padding:11px}.reports-reset{min-width:72px}.reports-insights{gap:8px}.reports-type-list>div{grid-template-columns:90px 1fr 24px}}
                </style>
