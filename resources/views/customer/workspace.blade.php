            ...
            @if($section === 'notifications')
                ...
                <div class="notification-page">
                    <div class="page-head notification-head">
                        ...
                        <button type="button" class="notification-read-all" style="{{ $notificationTotal ? '' : 'display:none' }}">✓ Mark all as read</button>
                    </div>
                    ...
                    <div class="card notification-empty" style="{{ $notificationTotal ? 'display:none' : '' }}">
                    ...
                    <section class="notification-layout" style="{{ $notificationTotal ? '' : 'display:none' }}">
                                ...
                                <div id="notification-list">
                                    @foreach($alerts as $alert)
                                        ...
                                        <article class="notification-row {{ $severity==='HIGH'?'critical':($severity==='MEDIUM'?'warning':'') }}" data-notification-row data-type="{{ strtolower((string)($alert->type ?? 'other')) }}" data-search="{{ strtolower(trim(($alert->title ?? '').' '.($alert->type ?? '').' '.($alert->severity ?? ''))) }}">
                                            ...
                                            <div class="notification-row-copy"><div class="notification-row-top"><strong>{{ $alert->title }}</strong><small>{{ $alert->created_at?->diffForHumans() ?? ($alert->due_date?->format('d M Y') ?? 'Active') }}</small></div><p>{{ ucwords(str_replace(['_','-'],' ',(string)$alert->type)) }}</p><span class="notification-meta">{{ $alert->due_date ? 'Due '.$alert->due_date->format('d M Y') : '' }}</span></div>
