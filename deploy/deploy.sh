...
for migration in \
  ...
  database/migrations/2026_08_22_000031_build_warehouse_field_inventory_foundation.php; do
...
echo "==> Verifying EAM and warehouse routes and commands"
...
echo "==> Verifying operational and warehouse tables"
php -r '
$db=new PDO("sqlite:".getcwd()."/database/database.sqlite");
$required=["customer_conversations","customer_messages","asset_failures","work_order_checklist_results","asset_spare_parts","stock_balances","warehouses","warehouse_bins","inventory_transfer_orders","inventory_transfer_order_lines","warehouse_user_assignments"];
...
if(count($tables)!==count($required)){fwrite(STDERR,"ERROR: one or more required operational/warehouse tables are missing after migrate\n");exit(1);}
...
echo "Operational and warehouse foundation ready\n";
'
