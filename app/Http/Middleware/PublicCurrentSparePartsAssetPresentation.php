...
class PublicCurrentSparePartsAssetPresentation
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $style = <<<'HTML'
<style id="unifco-current-spare-parts-asset-v2">
...
HTML;
        ...
        $script = <<<'HTML'
<script id="unifco-current-spare-parts-asset-script-v2">
(()=>{
 ...
 const target=()=>service.value==='quotation'&&subtype.value==='parts'&&!!assetId.value&&!$('uf-manual')?.classList.contains('show');
 const assetSnapshot=()=>({id:String(assetId.value||''),code:($('ac-code')?.textContent||assetList.selectedOptions[0]?.textContent?.split('—')[0]||'').trim(),name:($('ac-name')?.textContent||assetList.selectedOptions[0]?.textContent?.split('—').slice(1).join('—')||'أصل مسجل').trim()});
 ...
 service.addEventListener('change',after);subtype.addEventListener('change',after);assetList.addEventListener('change',after);$('site_id')?.addEventListener('change',after);$('asset-find')?.addEventListener('click',()=>setTimeout(activate,300));
 contract?.addEventListener('change',()=>{selectedAssets.clear();partsByAsset.clear();selectedParts.clear();loadingAssets.clear();after()});
