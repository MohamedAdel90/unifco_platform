...
class PublicRequestBottomTicketPresentation
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $styles = <<<'HTML'
<style id="unifco-request-bottom-ticket-v3">
.request-bottom-actions-row{display:flex!important;align-items:center!important;justify-content:space-between!important;gap:20px!important;width:100%!important;margin:18px 0 24px!important;padding:18px 0 0!important;border-top:1px solid #eef2f7!important;direction:ltr!important}
...
.request-bottom-actions-row .submit,.request-bottom-actions-row .emergency-submit,.request-bottom-actions-row button[type="submit"],.request-bottom-actions-row input[type="submit"]{width:150px!important;min-width:150px!important;max-width:150px!important;height:46px!important;min-height:46px!important;margin:0!important;border-radius:8px!important;font-size:12px!important;padding:0 16px!important;display:inline-flex!important;align-items:center!important;justify-content:center!important;background:#e3132c!important;color:#fff!important;border:0!important;font-weight:900!important;box-shadow:none!important}
...
.emergency-submit-row.request-bottom-actions-row{margin-top:18px!important}
@media(max-width:760px){.request-bottom-actions-row{flex-direction:column!important;align-items:stretch!important;direction:rtl!important}.request-bottom-actions-row .request-ticket-note{order:1!important;width:100%!important}.request-bottom-actions-row .request-actions-left{order:2!important;width:100%!important;direction:rtl!important}.request-bottom-actions-row .request-actions-left>*{flex:1 1 0!important;width:auto!important;max-width:none!important;min-width:0!important}}
</style>
HTML;
        $html = str_replace('</head>', $styles.'</head>', $html);

        /*
         * Server-side canonical footer for the current-maintenance routine form.
         * This deliberately matches the current submit button with or without id/extra attributes,
         * so the layout does not depend on JavaScript executing in the browser.
         */
        ...
        $html = preg_replace_callback($routinePattern, function ($m) use ($ticketNote, $cancel) {
            $submit = preg_replace('/\s+style="[^"]*"/i', '', $m[2]) ?? $m[2];
            return $m[1].'<div class="request-bottom-actions-row"><div class="request-actions-left">'.$submit.$cancel.'</div>'.$ticketNote.'</div>';
        }, $html, 1) ?? $html;

        /* Canonical emergency footer: keep one submit + one cancel only. */
        ...
        /* Remove top ticket note only when a bottom note was successfully created. */
        ...
        $script = <<<HTML
<script id="unifco-request-bottom-ticket-script-v3">
(()=>{
  ...
  const normalizeRow=(row)=>{
    ...
    const submits=[...row.querySelectorAll('.emergency-submit,.submit,button[type="submit"],input[type="submit"]')];
  ...
  const fallbackForm=(form)=>{
    ...
    if(!submit)return;
    let row=submit.closest('.request-bottom-actions-row,.submit-reference-row,.form-actions,.actions');
  ...
  const init=()=>{
    ...
    document.querySelectorAll('form[action*="service-requests"],form[action*="request"]').forEach(f=>{if(!f.querySelector('.request-bottom-actions-row'))fallbackForm(f)});
  };
