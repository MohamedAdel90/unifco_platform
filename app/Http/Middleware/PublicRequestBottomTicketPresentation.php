...
class PublicRequestBottomTicketPresentation
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $styles = <<<'HTML'
<style id="unifco-request-bottom-ticket-v2">
...
HTML;
        ...
        $script = <<<HTML
<script id="unifco-request-bottom-ticket-script-v2">
(()=>{
  ...
  const enhanceForm=(form)=>{
    ...
    /* Remove every legacy/duplicate cancel first. One canonical cancel is recreated below. */
    form.querySelectorAll('.emergency-cancel,.cancel-request-btn').forEach(el=>el.remove());

    let row=form.querySelector('.request-bottom-actions-row, .submit-reference-row, .emergency-submit-row');
    if(!row){
    ...
    let note=[...document.querySelectorAll('.request-ticket-note')].find(el=>!row.contains(el));
    if(!note) note=row.querySelector('.request-ticket-note');
  ...
  const init=()=>{
    ...
    if(forms.length) forms.forEach(enhanceForm);
    else [...document.querySelectorAll('form')].filter(f=>f.querySelector('.emergency-submit,.submit,button[type="submit"],input[type="submit"]')).forEach(enhanceForm);
  };
