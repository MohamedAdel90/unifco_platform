...
{
    public function __invoke(Request $request, AssetSparePartReorderService $reorder, AuthorizationService $authorization): View
    {
        $user=auth()->user();
        ...
        if($customerId && !Customer::whereKey($customerId)->exists()) $customerId=null;
        if($siteId && !CustomerSite::whereKey($siteId)->when($customerId,fn($q)=>$q->where('customer_id',$customerId))->exists()) $siteId=null;
        ...
        $sites=CustomerSite::query()->when($customerId,fn($q)=>$q->where('customer_id',$customerId))->orderBy('name')->get();
        ...
        $workOrders=WorkOrder::with('asset')
            ->when($customerId || $siteId || $contractId,fn($q)=>$q->whereIn('asset_id',$assetIds))
        ...
        $activePlans=MaintenancePlan::where('status','ACTIVE')
            ->when($customerId || $siteId || $contractId,fn($q)=>$q->whereIn('asset_id',$assetIds))
            ->when($contractId,fn($q)=>$q->where('service_contract_id',$contractId))->get();
        ...
        $stockAlerts=$reorder->alerts($customerId,$siteId)
            ->when($contractId,fn($rows)=>$rows->whereIn('asset_id',$assetIds))
            ->filter(fn($a)=>$a->computed_alert_status!=='OK')->values();
        ...
        $comparison=[
            ...
            'opened_wo'=>$this->comparison(
                $workOrders->filter(fn($w)=>$w->created_at && $w->created_at->gte($currentMonthStart))->count(),
                $workOrders->filter(fn($w)=>$w->created_at && $w->created_at->between($previousMonthStart,$previousMonthEnd))->count()
            ),
            'completed_wo'=>$this->comparison(
                $workOrders->filter(fn($w)=>$w->completed_at && $w->completed_at->gte($currentMonthStart))->count(),
                $workOrders->filter(fn($w)=>$w->completed_at && $w->completed_at->between($previousMonthStart,$previousMonthEnd))->count()
            ),
            'requests'=>$this->comparison(
                $serviceRequests->filter(fn($r)=>$r->created_at && $r->created_at->gte($currentMonthStart))->count(),
                $serviceRequests->filter(fn($r)=>$r->created_at && $r->created_at->between($previousMonthStart,$previousMonthEnd))->count()
            ),
        ];
        ...
        $recentActivity=$this->recentActivity($workOrders,$serviceRequests,$serviceUrl);
        ...
        $trend=$this->monthlyWorkOrderTrend($assetIds,$contractId,$customerId||$siteId||$contractId);

        $unreadNotifications=PlatformNotification::where('user_id',$user->id)->whereNull('read_at')->count();
        $filterActive=(bool)($customerId||$siteId||$contractId);

        return view('dashboard',compact(
            'days','dashboardMode','capabilities','customerId','siteId','contractId','customers','sites','contractOptions','filterActive','unreadNotifications','comparison',
            'operationalScore','operationalBand','averageAssetHealth','pmCompliance','slaPerformance','openWorkOrders','overdueWorkOrders','criticalWorkOrders','criticalAssets',
            'replacementAssets','stockAlerts','outOfStock','newServiceRequests','openServiceRequests','slaBreaches','outstandingReceivables','overdueReceivables','revenueThisMonth',
            'pendingPOs','pendingPOValue','activeEmployees','activeCustomers','expiringContracts','pmDue30','actionItems','recentActivity','topRiskAssets','recentWorkOrders',
            'statusDistribution','trend','assets','contracts'
        ));
    }

    private function dashboardMode(string $role,array $capabilities): string
    {
        if($role==='ADMIN') return 'EXECUTIVE';
        if($capabilities['maintenance'] && $capabilities['eam']) return 'MAINTENANCE';
        if($capabilities['finance']) return 'FINANCE';
        if($capabilities['procurement'] || $capabilities['inventory']) return 'SUPPLY_CHAIN';
        if($capabilities['crm']) return 'CUSTOMER_SERVICE';
        return 'OPERATIONS';
    }
    ...
    private function isSlaBreached(ServiceRequest $request): bool
    {
        ...
        $response=$request->response_sla_minutes && !$request->responded_at && $created->copy()->addMinutes((int)$request->response_sla_minutes)->isPast();
        $resolution=$request->resolution_sla_minutes && !$request->resolved_at && $created->copy()->addMinutes((int)$request->resolution_sla_minutes)->isPast();
    ...
    private function recentActivity(Collection $workOrders,Collection $requests,string $serviceUrl): Collection
    {
        $items=collect();
        foreach($workOrders->take(8) as $wo){
            $items->push((object)['time'=>$wo->updated_at,'type'=>'WORK_ORDER','title'=>$wo->work_order_no.' · '.($wo->asset?->asset_code ?: 'Asset'),'detail'=>$wo->maintenance_type.' · '.$wo->status,'url'=>route('maintenance.work-orders.show',$wo)]);
        }
        foreach($requests->take(8) as $r){
            $items->push((object)['time'=>$r->updated_at,'type'=>'SERVICE_REQUEST','title'=>$r->request_no.' · '.($r->subject ?: $r->service_category),'detail'=>$r->priority.' · '.$r->status,'url'=>$serviceUrl]);
        }
    ...
    private function monthlyWorkOrderTrend(Collection $assetIds,?int $contractId,bool $scoped): array
    {
        ...
        for($i=5;$i>=0;$i--){
            ...
            $opened[]=(clone $base)->whereBetween('created_at',[$start,$end])->count();
            $completed[]=(clone $base)->whereBetween('completed_at',[$start,$end])->count();
        }
