...
{
    public function __invoke(Request $request, AssetSparePartReorderService $reorder, AuthorizationService $authorization): View|RedirectResponse
    {
        $user=auth()->user();
        ...
        return view('dashboard',compact('days','dashboardMode','capabilities','customerId','siteId','contractId','customers','sites','contractOptions','filterActive','unreadNotifications','comparison','operationalScore','operationalBand','averageAssetHealth','pmCompliance','slaPerformance','openWorkOrders','overdueWorkOrders','criticalWorkOrders','criticalAssets','replacementAssets','stockAlerts','outOfStock','newServiceRequests','openServiceRequests','slaBreaches','outstandingReceivables','overdueReceivables','revenueThisMonth','pendingPOs','pendingPOValue','activeEmployees','activeCustomers','expiringContracts','pmDue30','actionItems','recentActivity','topRiskAssets','recentWorkOrders','statusDistribution','trend','assets','contracts'));
    }
