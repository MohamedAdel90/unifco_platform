...
class UserAdministrationController extends Controller
{
    ...
    public function store(Request $request,AuditService $audit): RedirectResponse
    {
        ...
        if(!empty($data['organization_id'])) abort_unless(Organization::where('tenant_id',$tenant)->whereKey($data['organization_id'])->exists(),422);
        if(!empty($data['employee_id'])) abort_unless(Employee::where('tenant_id',$tenant)->whereKey($data['employee_id'])->exists(),422);
        $user=User::create(['tenant_id'=>$tenant,'organization_id'=>$data['organization_id']?:null,'employee_id'=>$data['employee_id']?:null,'name'=>$data['name'],'email'=>$data['email'],'password'=>$data['password'],'role'=>$data['role'],'status'=>$data['status']]);
        $audit->record('security.user.created',$user,[],array_diff_key($user->toArray(),['password'=>true]));
    ...
    public function update(Request $request,int $user,AuditService $audit): RedirectResponse
    {
        ...
        if(!empty($data['organization_id'])) abort_unless(Organization::where('tenant_id',$tenant)->whereKey($data['organization_id'])->exists(),422);
        if(!empty($data['employee_id'])) abort_unless(Employee::where('tenant_id',$tenant)->whereKey($data['employee_id'])->exists(),422);
        $managed->update(['name'=>$data['name'],'email'=>$data['email'],'role'=>$data['role'],'status'=>$data['status'],'organization_id'=>$data['organization_id']?:null,'employee_id'=>$data['employee_id']?:null]);
