...
class CustomerAssetReadController extends Controller
{
    ...
    public function asset(Asset $asset): View
    {
        ...
        abort_unless((int)$asset->customer_id===$customerId,404);
        ...
        for($i=1;$i<$chronological->count();$i++){
            if(!$chronological[$i-1]->restored_at) continue;
            $start=\Carbon\Carbon::parse($chronological[$i-1]->restored_at);
        ...
        $pm=$workOrders->where('maintenance_type','PREVENTIVE');
        $pmCompliance=$pm->count()?round($pm->where('status','COMPLETED')->count()/$pm->count()*100,1):100;
        $metrics=compact('failureCount','totalDowntime','mttr','mtbf','completed','pmCompliance');

        return view('customer.asset-detail',compact('asset','plans','workOrders','failures','documents','specifications','attachments','metrics'));
    }

    public function workOrder(WorkOrder $workOrder): View
    {
        $customerId=$this->customerId();
        $workOrder->load(['asset.customer','asset.site','plan','contract']);
        abort_unless($workOrder->asset && (int)$workOrder->asset->customer_id===$customerId,404);
        ...
        if($workOrder->maintenance_plan_id){
            $tasks=DB::table('maintenance_plan_tasks')
                ...
                ->select('maintenance_plan_tasks.task_title','maintenance_plan_tasks.response_type','maintenance_plan_tasks.unit','work_order_checklist_results.result_status','work_order_checklist_results.numeric_value','work_order_checklist_results.text_value','work_order_checklist_results.completed_at')
                ->get();
        }
        ...
        $materials=DB::table('maintenance_materials')->join('items','items.id','=','maintenance_materials.item_id')
            ->where('maintenance_materials.work_order_id',$workOrder->id)
            ->select('maintenance_materials.quantity','items.item_code','items.name as item_name','items.uom')->get();
        $failures=DB::table('asset_failures')->where('work_order_id',$workOrder->id)
            ->select('failure_mode','failure_effect','corrective_action','failed_at','restored_at','downtime_minutes','severity','status')->get();

        return view('customer.work-order-detail',compact('workOrder','tasks','attachments','materials','failures'));
    }

    public function attachment(WorkOrder $workOrder,int $attachment)
    {
        $customerId=$this->customerId();
        $workOrder->load('asset');
        abort_unless($workOrder->asset && (int)$workOrder->asset->customer_id===$customerId,404);
        $file=DB::table('work_order_attachments')->where('id',$attachment)->where('work_order_id',$workOrder->id)->whereIn('attachment_type',['PHOTO','BEFORE_PHOTO','AFTER_PHOTO','THERMAL_IMAGE'])->first();
