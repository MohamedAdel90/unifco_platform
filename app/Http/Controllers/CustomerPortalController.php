...
class CustomerPortalController extends Controller
{
    public function __invoke(Request $request): View
    {
        ...
        $plans = MaintenancePlan::whereIn('asset_id',$assetIds)
            ->when($contractFilter, fn($q)=>$q->where('service_contract_id',$contractFilter))
            ->orderBy('next_due_date')->get();
        $workOrders = WorkOrder::whereIn('asset_id',$assetIds)
            ->when($contractFilter, fn($q)=>$q->where('service_contract_id',$contractFilter))
        ...
        foreach ($plans->whereNotNull('next_due_date')->filter(fn($p)=>$p->next_due_date->lte(now()->addDays(30))) as $p) {
            $alerts->push((object)['type'=>'MAINTENANCE_DUE','title'=>'صيانة مستحقة: '.$p->plan_no,'due_date'=>$p->next_due_date,'severity'=>$p->next_due_date->isPast()?'HIGH':'INFO']);
        }
        foreach ($invoices->filter(fn($i)=>$i->open_amount>0 && $i->due_date && $i->due_date->lte(now()->addDays(14))) as $i) {
            $alerts->push((object)['type'=>'INVOICE_DUE','title'=>'فاتورة مستحقة: '.$i->document_no,'due_date'=>$i->due_date,'severity'=>$i->due_date->isPast()?'HIGH':'INFO']);
        }
        foreach ($contracts->filter(fn($c)=>$c->ends_on && $c->ends_on->lte(now()->addDays(60))) as $c) {
            $alerts->push((object)['type'=>'CONTRACT_EXPIRY','title'=>'قرب انتهاء العقد: '.$c->contract_no,'due_date'=>$c->ends_on,'severity'=>'INFO']);
        }
        ...
        return view('customer.portal',compact(
            ...
            'openInvoiceAmount','openWorkOrders','preventiveCount','correctiveCount','contractFilter','assetFilter','locationFilter'
        ));
    }
}
