...
{
    public function show(Request $request, ServiceRequest $serviceRequest, ScopeService $scopes, AuthorizationService $authorization): View
    {
        $user = $request->user();
        ...
        $step = ApprovalRequest::query()->where('tenant_id',$serviceRequest->tenant_id)
            ->where('entity_type',ServiceRequest::class)->where('entity_id',$serviceRequest->id)
            ->where('action',$serviceRequest->workflow_stage)->first();
        $roles = $authorization->roleCodes($user)->push(strtoupper((string)$user->role))->filter()->unique();
        $canAct = $step && $step->status === 'PENDING' && $roles->contains(strtoupper((string)$step->approval_role));
        if($serviceRequest->workflow_stage === 'EXECUTION') $canAct = $canAct && (int)$serviceRequest->assigned_engineer_id === (int)$user->id;

        $technicians = collect();
        if($serviceRequest->workflow_stage === 'TECHNICIAN_ASSIGNMENT' && $canAct){
            $technicians = User::query()->where('tenant_id',$user->tenant_id)->whereIn('status',['ACTIVE','ENABLED'])
                ->whereIn('role',['TECHNICIAN','MAINTENANCE_ENGINEER'])->orderBy('name')->get(['id','name','role']);
        }
