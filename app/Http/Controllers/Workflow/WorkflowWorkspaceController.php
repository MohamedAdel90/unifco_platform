...
class WorkflowWorkspaceController extends Controller
{
    ...
    private function attention(string $role): array
    {
        ...
        $open=fn()=>ServiceRequest::whereNotIn('status',['CLOSED','REJECTED','CANCELLED']);
        return [
            ['key'=>'breached','label'=>'SLA Breached','value'=>ApprovalRequest::where('approval_role',$role)->where('status','PENDING')->whereNotNull('due_at')->where('due_at','<',now())->count(),'tone'=>'red'],
            ['key'=>'emergency','label'=>'Emergency Open','value'=>$open()->where('priority','EMERGENCY')->count(),'tone'=>'red'],
            ['key'=>'review','label'=>'Awaiting My Review','value'=>ApprovalRequest::where('approval_role',$role)->where('status','PENDING')->count(),'tone'=>'amber'],
            ['key'=>'workorder','label'=>'Work Orders In Progress','value'=>WorkOrder::whereNotIn('status',['COMPLETED','CLOSED','CANCELLED'])->count(),'tone'=>'blue'],
        ];
    }
    ...
    private function metrics(string $role): array
    {
        ...
        return match($role){
            ...
            'MAINTENANCE_MANAGER'=>[['label'=>'Manager Approvals','value'=>$pending()],['label'=>'Requests Under Review','value'=>ServiceRequest::whereIn('workflow_stage',['MAINTENANCE_MANAGER_REVIEW','TECHNICAL_ASSESSMENT'])->count()],['label'=>'Open Work Orders','value'=>WorkOrder::whereNotIn('status',['COMPLETED','CLOSED','CANCELLED'])->count()],['label'=>'SLA Breaches','value'=>$breached()]],
            ...
            'FINANCE_MANAGER'=>[['label'=>'Finance Approvals','value'=>$pending()],['label'=>'Open Receivables','value'=>(float)FinancialDocument::where('document_type','AR_INVOICE')->where('open_amount','>',0)->sum('open_amount'),'money'=>true],['label'=>'Long-Term Quotations','value'=>CrmQuotation::where('payment_terms_days','>',30)->whereNotIn('status',['CUSTOMER_REJECTED','EXPIRED'])->count()],['label'=>'SLA Breaches','value'=>$breached()]],
            'PROJECT_MANAGER'=>[['label'=>'Execution Approvals','value'=>$pending()],['label'=>'Execution Review Requests','value'=>ServiceRequest::where('workflow_stage','PROJECT_MANAGER_REVIEW')->count()],['label'=>'Open Work Orders','value'=>WorkOrder::whereNotIn('status',['COMPLETED','CLOSED','CANCELLED'])->count()],['label'=>'SLA Breaches','value'=>$breached()]],
            'CEO'=>[['label'=>'Executive Approvals','value'=>$pending()],['label'=>'High Risk Quotations','value'=>CrmQuotation::where('risk_level','HIGH')->whereNotIn('status',['CUSTOMER_REJECTED','EXPIRED'])->count()],['label'=>'High Value Quotations','value'=>CrmQuotation::where('amount','>',250000)->whereNotIn('status',['CUSTOMER_REJECTED','EXPIRED'])->count()],['label'=>'Approval SLA Breaches','value'=>$breached()]],
        };
    }
    ...
    private function workQueue(string $role): Collection
    {
        ...
        return match($role){
            'MAINTENANCE_MANAGER'=>ServiceRequest::whereIn('workflow_stage',['MAINTENANCE_MANAGER_REVIEW','TECHNICAL_ASSESSMENT'])->latest()->limit(8)->get()->map(fn($r)=>['type'=>'Technical Scope','title'=>$r->request_no.' · '.$r->subject,'meta'=>$r->company_name.' · '.$r->priority,'state'=>str_replace('_',' ',$r->workflow_stage),'url'=>route('workflow.approvals.index')]),
            ...
            'FINANCE_MANAGER'=>CrmQuotation::where('payment_terms_days','>',30)->whereNotIn('status',['CUSTOMER_REJECTED','EXPIRED'])->latest('quotation_date')->limit(8)->get()->map(fn($q)=>['type'=>'Financial Terms','title'=>$q->quotation_no,'meta'=>number_format((float)$q->amount,2).' '.$q->currency.' · '.$q->payment_terms_days.' days','state'=>$q->risk_level,'url'=>route('finance.core.index')]),
            'PROJECT_MANAGER'=>ServiceRequest::whereIn('workflow_stage',['PROJECT_MANAGER_REVIEW','TECHNICIAN_ASSIGNMENT','EXECUTION'])->latest()->limit(8)->get()->map(fn($r)=>['type'=>'Execution','title'=>$r->request_no.' · '.$r->subject,'meta'=>$r->company_name.' · '.$r->priority,'state'=>str_replace('_',' ',$r->workflow_stage),'url'=>route('projects.projects.index')]),
