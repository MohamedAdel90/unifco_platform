...
class WorkflowWorkspaceController extends Controller
{
    ...
    private function metrics(string $role): array
    {
        ...
        return match($role){
            ...
            'PROJECT_MANAGER'=>[['label'=>'Execution Approvals','value'=>$pending()],['label'=>'Execution Review Requests','value'=>ServiceRequest::where('workflow_stage','EXECUTION_FEASIBILITY')->count()],['label'=>'Open Work Orders','value'=>WorkOrder::whereNotIn('status',['COMPLETED','CLOSED','CANCELLED'])->count()],['label'=>'SLA Breaches','value'=>$breached()]],
