...
class AssetMasterController extends Controller
{
    ...
    {
        $user=$this->checker($request); $data=$request->validate(['customer_id'=>['required',Rule::exists('customers','id')->where(fn($q)=>$q->where('tenant_id',$user->tenant_id))],'customer_site_id'=>['required',Rule::exists('customer_sites','id')],'parent_id'=>['nullable','integer'],'location_type'=>['required',Rule::in(AssetMasterService::LOCATION_TYPES)],'code'=>['required','string','max:80'],'name'=>['required','string','max:160'],'description'=>['nullable','string','max:255'],'latitude'=>['nullable','numeric','between:-90,90'],'longitude'=>['nullable','numeric','between:-180,180']]);
        $site=CustomerSite::with('customer')->findOrFail($data['customer_site_id']); abort_unless($site->customer && (int)$site->customer->tenant_id===(int)$user->tenant_id && (int)$site->customer_id===(int)$data['customer_id'],422,'Selected site does not belong to the selected customer.');
