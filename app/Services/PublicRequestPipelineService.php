...
class PublicRequestPipelineService
{
    ...
    public function convert(PublicServiceRequest $public): PublicServiceRequest
    {
        return DB::transaction(function () use ($public) {
            ...
            $requestType = match (strtoupper((string) $public->request_type)) {
                'QUOTATION' => 'QUOTATION',
            ...
            $contract = ServiceContract::where('customer_id', $customer->id)
                ->where('status', 'ACTIVE')
            ...
            $serviceRequest = ServiceRequest::create([
                'tenant_id'=>$tenant->id,'organization_id'=>$org->id,'customer_id'=>$customer->id,
                'service_contract_id'=>$contract?->id,'asset_id'=>$asset?->id,'request_no'=>'SR-'.$public->reference_no,'request_type'=>$requestType,
                'company_name'=>$public->company_name,'commercial_registration'=>$public->commercial_registration,'email'=>$public->email,'mobile'=>$public->mobile,
                'service_category'=>$public->service_category,'subject'=>$public->subject,'details'=>$public->details."\n\n".implode("\n", $meta),
                'site_city'=>$public->site_city,'priority'=>$priority,'status'=>'OPEN','workflow_stage'=>'NEW','eligibility'=>$eligibility,
                'response_sla_minutes'=>$priority === 'EMERGENCY' ? 10 : 120,
                'resolution_sla_minutes'=>$priority === 'EMERGENCY' ? 240 : 1440,
            ]);
            $links['service_request_id'] = $serviceRequest->id;

            $this->customers->record($customer, 'SERVICE_REQUEST_CREATED', 'Service request '.$serviceRequest->request_no.' created', $public->subject, $serviceRequest, [
                'request_type'=>$requestType,'priority'=>$priority,'eligibility'=>$eligibility,
            ]);
            ...
            if (in_array($requestType, ['QUOTATION','CONSULTATION'], true) || ($requestType === 'MAINTENANCE' && ! $contract && $priority !== 'EMERGENCY')) {
                ...
                if ($needsQuotation) {
                    $quotation = CrmQuotation::create([
                        'tenant_id'=>$tenant->id,'organization_id'=>$org->id,'opportunity_id'=>$opportunity->id,'customer_id'=>$customer->id,
                        'quotation_no'=>'QT-'.$public->reference_no,'revision_no'=>0,'quotation_date'=>now()->toDateString(),'currency'=>'SAR','amount'=>0,'cost_amount'=>0,
                        'risk_level'=>'NORMAL','status'=>'DRAFT',
                    ]);
            ...
            if ($requestType === 'MAINTENANCE' && ($contract || $priority === 'EMERGENCY')) {
                $workOrder = WorkOrder::create([
                    'tenant_id'=>$tenant->id,'organization_id'=>$org->id,'customer_id'=>$customer->id,'service_contract_id'=>$contract?->id,
                    'work_order_no'=>'WO-'.$public->reference_no,'asset_id'=>$asset->id,'maintenance_type'=>'CORRECTIVE','priority'=>$priority,'status'=>'OPEN','planned_start'=>$plannedStart,
                ]);
            ...
            $this->workflow->start($serviceRequest->fresh(), [
                'estimated_value'=>0,
                'margin_pct'=>null,
                'payment_terms_days'=>0,
                'risk_level'=>'NORMAL',
                'procurement_required'=>false,
                'paid'=>$requestType === 'QUOTATION',
            ]);
