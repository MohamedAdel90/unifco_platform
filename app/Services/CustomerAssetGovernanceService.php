...
class CustomerAssetGovernanceService
{
    ...
    public function review(User $checker,int $submissionId,bool $approve,?string $notes=null): ?Asset
    {
        ...
        $data=['customer_id'=>$submission->customer_id,'customer_site_id'=>$submission->customer_site_id,'name'=>$submission->name,'customer_asset_code'=>$submission->customer_asset_code,'serial_no'=>$submission->serial_no,
            ...
            'maintenance_strategy'=>$submission->maintenance_strategy ?: 'PREVENTIVE'];
    ...
    private function normalizeRow(array $row): array
    {
        ...
        foreach(['name','asset_category','customer_site_id','asset_type','manufacturer','model_no','serial_no','physical_location','installation_date'] as $required) abort_if(empty($r[$required]),422,$required.' is required.');
        ...
        return ['name'=>$r['name'],'asset_category'=>$r['asset_category'],'customer_site_id'=>(int)$r['customer_site_id'],'customer_asset_code'=>$r['customer_asset_code']??null,'serial_no'=>$r['serial_no'],
            'asset_type'=>$r['asset_type'],'manufacturer'=>$r['manufacturer'],'model_no'=>$r['model_no'],'criticality'=>strtoupper($r['criticality']??'MEDIUM'),'ownership_type'=>'CUSTOMER_OWNED',
            'maintenance_strategy'=>strtoupper($r['maintenance_strategy']??'PREVENTIVE'),'installation_date'=>$r['installation_date'],'physical_location'=>$r['physical_location'],'technical_specifications'=>$spec];
    }
