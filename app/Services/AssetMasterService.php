...
class AssetMasterService
{
    ...
    private const TRANSITIONS=[
        ...
        'OUT_OF_SERVICE'=>['ACTIVE','UNDER_MAINTENANCE','DECOMMISSIONED'],
    ...
    public function create(int $tenantId,int $organizationId,int $userId,array $data): Asset
    {
        ...
        return DB::transaction(function() use($tenantId,$organizationId,$userId,$data){
            ...
            $asset=Asset::create([
                ...$data,'tenant_id'=>$tenantId,'organization_id'=>$organizationId,
                'asset_code'=>$data['asset_code']??('AST-'.str_pad((string)$next,6,'0',STR_PAD_LEFT)),'qr_token'=>$data['qr_token']??Str::random(40),
                'lifecycle_status'=>'PENDING_VERIFICATION','operational_status'=>$data['operational_status']??'STANDBY','verification_status'=>'PENDING',
                'commissioning_status'=>'NOT_STARTED','status'=>'REGISTERED',
            ]);
            $this->event($asset,'ASSET_REGISTERED',null,'PENDING_VERIFICATION','Asset registered','Asset master record created and sent for verification.',$userId);
    ...
    public function verify(Asset $asset,int $userId,?string $notes=null): Asset
    {
        ...
        $asset->update(['verification_status'=>'VERIFIED','verified_by'=>$userId,'verified_at'=>now(),'verification_notes'=>$notes,'lifecycle_status'=>'ACTIVE','operational_status'=>$asset->operational_status==='STANDBY'?'ACTIVE':$asset->operational_status,'status'=>'ACTIVE']);
        $this->event($asset,'ASSET_VERIFIED',$from,'ACTIVE','Asset verified and activated',$notes,$userId);
        return $asset->refresh();
    }

    public function transition(Asset $asset,string $toStatus,int $userId,?string $notes=null): Asset
    {
        $toStatus=strtoupper($toStatus); $from=$asset->lifecycle_status;
        abort_if($from==='DISPOSED',422,'Disposed assets cannot transition.');
        abort_unless(in_array($toStatus,self::TRANSITIONS[$from]??[],true),422,'Invalid asset lifecycle transition: '.$from.' → '.$toStatus);
        $changes=['lifecycle_status'=>$toStatus,'status'=>$toStatus==='ACTIVE'?'ACTIVE':$toStatus];
        if($toStatus==='UNDER_MAINTENANCE') $changes['operational_status']='UNDER_MAINTENANCE';
        if($toStatus==='OUT_OF_SERVICE') $changes['operational_status']='OUT_OF_SERVICE';
        if($toStatus==='ACTIVE') $changes['operational_status']='ACTIVE';
        if($toStatus==='DECOMMISSIONED') $changes['operational_status']='DECOMMISSIONED';
        if($toStatus==='DISPOSED') $changes+=['operational_status'=>'DISPOSED','disposed_at'=>now()];
        $asset->update($changes);
        $this->event($asset,'LIFECYCLE_TRANSITION',$from,$toStatus,'Asset lifecycle changed',$notes,$userId);
        return $asset->refresh();
    }
    ...
    public function requestCommissioning(Asset $asset,int $userId,array $data): AssetCommissioningRecord
    {
        ...
        return DB::transaction(function() use($asset,$userId,$data){
            $record=AssetCommissioningRecord::create([
                'tenant_id'=>$asset->tenant_id,'organization_id'=>$asset->organization_id,'asset_id'=>$asset->id,'status'=>'PENDING_APPROVAL',
                'inspection_date'=>$data['inspection_date']??now()->toDateString(),'inspection_result'=>$data['inspection_result']??'PASS',
                'checklist'=>$data['checklist']??null,'notes'=>$data['notes']??null,'created_by'=>$userId,
            ]);
            $asset->update(['commissioning_status'=>'PENDING_APPROVAL','commissioning_requested_by'=>$userId,'commissioning_requested_at'=>now(),'commissioning_notes'=>$data['notes']??null]);
            $this->event($asset,'COMMISSIONING_REQUESTED',$asset->lifecycle_status,$asset->lifecycle_status,'Commissioning submitted for approval',$data['notes']??null,$userId,['commissioning_record_id'=>$record->id]);
            return $record;
        });
    }
