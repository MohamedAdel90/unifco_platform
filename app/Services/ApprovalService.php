...
class ApprovalService
{
    ...
    private function advanceServiceRequest(ServiceRequest $serviceRequest, ApprovalRequest $approval, string $decision, ?string $note): void
    {
        if ($decision === 'REJECTED') {
            ApprovalRequest::where('entity_type', ServiceRequest::class)
            ...
            $technical = ApprovalRequest::where('entity_type', ServiceRequest::class)
                ->where('entity_id', $serviceRequest->id)
            ...
            ApprovalRequest::where('entity_type', ServiceRequest::class)
                ...
                ->update(['status'=>'WAITING','due_at'=>null,'decided_by'=>null,'decided_at'=>null]);

            if ($technical) {
                $technical->update([
                    'status'=>'PENDING','due_at'=>now()->addMinutes($technical->sla_minutes ?: 120),
                    'decided_by'=>null,'decided_at'=>null,
                    'decision_note'=>trim('Returned for correction: '.($note ?: '')),
                ]);
            }
        ...
        if ($serviceRequest->customer_id) {
            ...
            if ($customer) {
                $this->customers->record(
                    $customer,
                    'APPROVAL_'.$decision,
                    str_replace('_',' ', $approval->action).' '.$decision,
                    $note,
                    $serviceRequest,
                    ['approval_role'=>$approval->approval_role,'step_order'=>$approval->step_order],
                    'INTERNAL'
                );
            }
        }
